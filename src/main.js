  ...
  async connectWallet(walletName) {
    ...
    try {
      ...
      if (isMobileUserAgent) {
        ...
        // Attempt to connect and get URI immediately
        let uri = null;
        try {
          ...
          console.error(`WalletConnect connect failed for ${walletName}:`, connectError);
          throw new Error(`Failed to connect WalletConnect: ${connectError.message}`);
        }

        if (!uri) {
          throw new Error('No URI generated by WalletConnect');
        }
        ...
        console.log(`Attempting direct deeplink for ${walletName}: ${deeplink}`);
        const deeplinkResult = window.location.href = deeplink;
        console.log(`Deeplink result for ${walletName}:`, deeplinkResult);

        // Fallback to QR code if deeplink doesn’t work after 1 second
        setTimeout(() => {
          if (document.visibilityState === 'visible') {
            console.log(`Deeplink failed or timed out for ${walletName} after 1 second`);
        ...
        // Wait for session and accounts (async in background)
